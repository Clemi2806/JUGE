...
public class PBTTestingTool implements ITestingTool {

    /**
     * The EvoSuite instance for this execution
     */
    private EvoSuite evosuitePBT;
    ...
    public List<File> getExtraClassPath() {
        ...
        File bbc = new File("lib", "evosuite-pbt.jar");
        if (!bbc.exists()) {
            System.err.println("Wrong EvoSuite-PBT jar setting, jar is not at: " + bbc.getAbsolutePath());
        } else {
            files.add(bbc);
        }
        return files;
    ...
    public void run(String cName, long timeBudget) {
        if (this.evosuitePBT == null) {
            System.err.println("EvoSuite-PBT has not been initialized with the target classpath!");
        } else {
            ...
            this.evosuitePBT.parseCommandLine(command);
        }
    }
}
